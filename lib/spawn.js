 ...
 * @returns {Promise}
 */
function spawn(phantomJsConfig, phantomJsOptions) {
    var configPath;
    var stdout;
    var stderr;
    var child;
     ...
     */
        .then(function startPhantom() {
            return when.promise(function (resolve, reject) {
                ...
                var runOptions = ["--config=" + configPath];
                for (var k in phantomJsOptions) {
                    runOptions.push(k + "=" + phantomJsOptions[k]);
                }
                child = childProcess.spawn(phantomjs.path, runOptions.concat([
                    startScript,
                    configPath
                ]));
