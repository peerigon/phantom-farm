 ...
 */
function init(tryNum) {
    var timer;

    var port = system.args.find(function (element) {
        return element.indexOf("--communication-port=") == 0;
    });

    socket = new window.WebSocket("ws://127.0.0.1:" + port.slice("--communication-port=".length) + "/");
